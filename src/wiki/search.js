...
const { sendNormalMessage, sendMentionMessage } = require('../discord/message')
const { sendErrorConsole } = require('../helper/utils')
const config = require('../../config.json')

exports.searchItemInWiki = (message, title, args) => {
    if (!args[1]) {
        sendMentionMessage('falto el nombre del item', message);
        return;
    }
    var itemName = "";
    ...
    getImage(url, guildName).then(result => {
        ...
        if (!result.success) {
            sendMentionMessage(`${title.noItemFound.stringValue} **${itemName}**`, message);
            return;
        }
        if (result.textblock) {
            outputString += `\n${result.textblock}`;
        ...
        sendMentionMessage(`${title.generalError.stringValue} en la búsqueda del item`, message)
...
let getImage = async (url) => {
    ...
    var paragraphs = await page.$(config.puppeteer.wikiParagraphsSelector);
